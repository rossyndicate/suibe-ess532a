...
You want to re-create the word document shared with this assignment titled `Debugged_Document.word`. Below is the workflow to do so, ***however*** you will notice if you attempt to render this R Markdown document as is, it does NOT reproduce the `Debugged_Document.word` file, and it won't even render until you fix some of the mistakes/missing elements. Therefore, this workflow is not reproducible! Your task is to fix the below coding workflow so that it renders and **exactly** matches the output of `Debugged_Document.word` (*including* whether or not certain code chunks and/or outputs are shown).
